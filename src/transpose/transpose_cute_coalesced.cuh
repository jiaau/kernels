...
template <typename T,
          ...
          class SmemLayoutD,
          class ThreadLayoutD>
__global__ void transpose_cute_coalesced_kernel(TensorS const S,
                                                TensorD const D,
...
void transpose_cute_coalesced(T *out, const T *in, const int64_t M, const int64_t N) {
    // Make Tensors
    ...
    // Q1: Sequential swapping does not affect the correctness of the result, but does affect the number of bank conflicts?
    // A1: 对于 gmem -> 按行读，按列写 -> smem 按行读，按行写 -> gmem 这种 transpose 方式，sD 元素和 gD 元素是完全对应的，只要二者使用的线程布局相同，
    //     那么线程布局的顺序对结果没有影响，影响的只是线程的访问位置，所以 bank conflicts 次数不同。
    // Q2: 为什么这里 layout 是相同的？ 
    // A2: 因为块内索引是相同的，参考：transpose_cuda_coalesced.cuh
    ...
    // key point:
    auto smemLayoutS = make_layout(block_shape_s, GenColMajor{});
    auto smemLayoutD = make_layout(block_shape_d, GenRowMajor{});
    // ? why this works? or:
    ...
    transpose_cute_coalesced_kernel<T><<<gridDim, blockDim>>>(
